...
func _process(_delta):
	if is_circular:
		if curr_val < 0:
			...
			$MeshInstance3D.rotation.x = PI
	elif is_solenoid:
		if curr_val < 0:
			for child in get_children():
				if child.get_node("MeshInstace3D"):
					child.get_node("MeshInstance3D").rotation.x = 0
		else:
			for child in get_children():
				if child.get_node("MeshInstace3D"):
					child.get_node("MeshInstance3D").rotation.x = PI
	else:
		if curr_val < 0:
			$MeshInstance3D.rotation.x = PI
		else:
			$MeshInstance3D.rotation.x = 0
		for arrow in get_node("Magnetic Field Direction").get_children():
			if curr_val < 0:
				arrow.rotation.x = PI + PI/2
			else:
